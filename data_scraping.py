...
def fetch_weather_data():
    try:
        #request data
        response = requests.get(dbinfo.weather_url, params={"appid": dbinfo.weather_api_key, "q": dbinfo.q_city, "lang": "en", "mode": "json"}, timeout=10)
        #check response status
        response.raise_for_status()
        #save data
...
updated_weather_data = fetch_weather_data()
with open("weather_data.txt", "w", encoding="utf-8") as f:
    json.dump(updated_weather_data, f, indent=4, ensure_ascii=False)
